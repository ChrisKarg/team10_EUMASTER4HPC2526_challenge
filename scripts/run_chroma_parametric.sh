#!/bin/bash
#
# Chroma Parametric Benchmark Automation Script
#
# This script automates the complete workflow for running parametric Chroma benchmarks:
# 1. Check for running Chroma service
...
echo "========================================================================"
echo "Chroma Parametric Benchmark Automation"
echo "========================================================================"
echo ""

# Step 1: Check for running Chroma service
echo "[1/6] Checking for running Chroma service..."
SERVICE_OUTPUT=$(python main.py --list-running-services 2>/dev/null || echo "")

if echo "$SERVICE_OUTPUT" | grep -q "chroma"; then
    # Extract service ID
    SERVICE_ID=$(echo "$SERVICE_OUTPUT" | grep "chroma" | head -1 | awk '{print $2}')
    echo "    ✓ Found Chroma service: $SERVICE_ID"
else
    echo "    ✗ No Chroma service found!"
    echo ""
    echo "Please start a Chroma service first:"
    echo "    python main.py --recipe recipes/services/chroma.yaml"
    echo ""
    read -p "Would you like to start Chroma now? (y/N): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
...
SUBMIT_OUTPUT=$(python main.py --recipe recipes/clients/chroma_parametric.yaml --target-service "$SERVICE_ID" 2>&1)
...
# Run plotting script
if python analysis/plot_chroma_results.py; then
    echo "    ✓ Plots generated successfully"
else
    echo "    ✗ Failed to generate plots"
    echo "    You can manually run: python analysis/plot_chroma_results.py"
    exit 1
fi
...
if [ -d "$PLOTS_DIR" ]; then
    ...
    ls -1 "$PLOTS_DIR"/*.png 2>/dev/null | sed 's/^/      /' || echo "      (No PNG files found yet)"
    echo ""
...
echo "    python analysis/plot_chroma_results.py --help"
echo ""
